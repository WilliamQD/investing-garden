...
export async function PUT(
  ...
  const { id } = await params;
  try {
    const rateLimit = await checkRateLimit('learning:update', { limit: 40, windowMs: 60_000 });
    if (!rateLimit.allowed) {
      return NextResponse.json(
        { error: 'Too many learning updates. Try again shortly.' },
        {
          status: 429,
          headers: {
    ...
    await logAuditEvent('learning_entry_updated', session, { entryId: entry.id });
    return NextResponse.json(entry);
  } catch (error) {
...
export async function DELETE(
  ...
  const { id } = await params;
  try {
    const rateLimit = await checkRateLimit('learning:delete', { limit: 30, windowMs: 60_000 });
    if (!rateLimit.allowed) {
      return NextResponse.json(
        { error: 'Too many learning updates. Try again shortly.' },
        {
    ...
    await logAuditEvent('learning_entry_deleted', session, { entryId: id });
