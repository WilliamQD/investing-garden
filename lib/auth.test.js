...
test('credential verification with multiple valid credentials (async)', async () => {
  const hashedPwd = await hashPassword('hashedpassword');
  const credentials = [
    { username: 'user1', password: 'password1' }, // Plaintext
    { username: 'user2', password: hashedPwd },   // Hashed
  ];
  
  // Simulate verifyCredentials logic (new async version)
  const verifyTest = async (username, password) => {
    const normalizedUsername = username.trim();
    if (!normalizedUsername || !password) return false;

    const credential = credentials.find(cred => safeEquals(cred.username, normalizedUsername));
    if (!credential) return false;

    return await verifyPassword(password, credential.password);
  };
  
  assert.equal(await verifyTest('user1', 'password1'), true);
  assert.equal(await verifyTest('user2', 'hashedpassword'), true);
  assert.equal(await verifyTest('user1', 'password2'), false);
  assert.equal(await verifyTest('user2', 'wrongpassword'), false);
  assert.equal(await verifyTest('user3', 'password1'), false);
  assert.equal(await verifyTest(' user1 ', 'password1'), true); // Trimmed
});
